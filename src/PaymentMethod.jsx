    ...
    return (
        <>
            {settings.payment_method_image != '' && (
                <img src={settings.payment_method_image} style={{ marginRight: '5px' }}  />
            )}
    ...
    useEffect(() => {
        return onCheckoutSuccess((response) => {
            if(response.redirect)
                return redirectToCheckout(response.redirect);
            else
                return openCheckoutModal(response.processingResponse.paymentDetails);
        });
    }, [onCheckoutSuccess]);
...
function openCheckoutModal(response) {
    ...
    let options = {
        ...
        onError: (error) => {
            location.href = response.return_url + '&fromCallback=onError&status=' + (mbbxPaymentData ? mbbxPaymentData.status.code : '500');
        },
    };
