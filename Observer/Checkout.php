...
class Checkout
{
    ...
    {
        if(!$this->helper->isReady() && $this->config->custom_dni != '')
            return $fields;

        //Enable own dni option if disabled
        ...
        $fields['billing_dni'] = [
            ...
            'default'     => WC()->session && !empty(WC()->session->get('mbbx_billing_dni')) ? WC()->session->get('mbbx_billing_dni') : get_user_meta($cutomer_id, 'billing_dni', true),
        ];
